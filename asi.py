...
try:
    ...
    SAFE_CHAT_ID = int(CFG.get("safe_chat_id", "0"))
    SAFE_MSG_ID = int(CFG.get("safe_msg_id", "0"))
    # --------------------------------
...
# ---------------- SAFE CHANNEL DOWNLOAD (FIX) ----------------
async def download_from_safe(app, file_name):
    """Worker safe-channel se video download karega."""
    if not SAFE_CHAT_ID or not SAFE_MSG_ID:
        raise Exception("Safe channel details missing in payload.")
    msg = await app.get_messages(SAFE_CHAT_ID, SAFE_MSG_ID)
    if not msg or not (msg.video or msg.document):
        raise Exception("Safe channel message not found or has no media.")
    output_path = f"/kaggle/working/{file_name}"   # <-- FIX: writable path
    reset_prog()
    result = await asyncio.wait_for(
        app.download_media(msg, file_name=output_path, progress=prog, progress_args=(app, "download")),
        timeout=1800
    )
    if not result or not os.path.exists(result):
        raise Exception("Download from safe channel returned empty.")
    return result

async def download_asset_robust(app, media_id, file_name, step_name):
    """Fallback safe download for watermark/font using file_id or safe channel copy."""
    if not media_id or media_id == "none":
        raise Exception(f"No media ID for '{step_name}'")
    output_path = f"/kaggle/working/{file_name}"
    try:
        reset_prog()
        result = await asyncio.wait_for(
            app.download_media(media_id, file_name=output_path, progress=prog, progress_args=(app, step_name)),
            timeout=600
        )
        ...
        pass
    # If direct file_id fails, try fetching from safe channel (if provided)
    if SAFE_CHAT_ID and SAFE_MSG_ID:
        try:
            msg = await app.get_messages(SAFE_CHAT_ID, SAFE_MSG_ID)
            # Check if the message's media file_id matches? Not reliable, so just attempt to download again via the message object
            # Actually, we can't recover here. Fallback to error.
        except:
            pass
    raise Exception(f"Download failed for '{step_name}'.")
...
async def deliver_video_asset(app_instance, chat_id, target_user, file_path, caption, progress_callback):
    ...
    thumb_path = "/kaggle/working/thumb.jpg"
    try:
...
async def main_driver():
    ...
    # ---------- FIX: Download from safe channel ----------
    video_file = await download_from_safe(app, "video.mkv")
    # ------------------------------------------------------
    if not video_file:
    ...
    if FONT_LINK and FONT_LINK != "none":
        font_path = await download_asset_robust(app, FONT_LINK, "custom_font.ttf", "font_download")
    ...
    if TASK_TYPE == "hardsub":
        if SUB_ID and SUB_ID != "none":
            sub_file = await download_asset_robust(app, SUB_ID, "sub_raw", "subtitle_download")
        ...
        if sub_file.lower().endswith('.ass') or is_ass_format(sub_file):
            ...
            if FONT_LINK and FONT_LINK != "none":
                ...
                with open("/kaggle/working/ready_sub.ass", "w", encoding="utf-8") as f:
                    f.write("\n".join(new_lines))
            else:
                shutil.copy(sub_file, "/kaggle/working/ready_sub.ass")
        else:
            try:
            ...
            new_subs.save("/kaggle/working/ready_sub.ass")

        if WM_ID and WM_ID != "none" and not has_watermark:
            wm_file = await download_asset_robust(app, WM_ID, "watermark.png", "watermark_download")
    ...
    if TASK_TYPE == "compress":
        ...
        if res_probe.stdout.strip():
            ...
            for i, st in enumerate(streams):
                ...
                if s_codec in ['ass', 'ssa']:
                    ass_out = f"/kaggle/working/{base_name}_track_{i+1}.ass"
                    ...
                    temp_sub = f"/kaggle/working/temp_{i+1}{temp_ext}"
        ...
        vf_filter = "subtitles='/kaggle/working/ready_sub.ass':charenc=UTF-8"
        if FONT_LINK and FONT_LINK != "none":
        ...
        if wm_file and os.path.exists(wm_file):
            ...
            cmd_cpu = ["ffmpeg", "-y", "-progress", "pipe:1", "-i", video_file, "-vf", v_filter,
                       ...
                       "-threads", "0", "-c:a", "aac", "-movflags", "+faststart", out_path]
    ...
    await deliver_video_asset(app_up, CHAT_ID, USER_ID, out_path, f"✅ Process Completed!\n`{out_name}`", prog)
