...
@web_app.get("/")
def read_root():
    return {"status": "Processing Controller Live"}
...
def load_server_accounts():
    ...
    while True:
        ...
        i += 1
    if not accs:
        raise RuntimeError("No Cloud credentials configured.")
    return accs

KAG_ACCOUNTS = load_server_accounts()
...
def run_server_command(account, cmd_args, task_ref_id, timeout=60):
    config_dir = Path(f"/tmp/node_config_{task_ref_id}")
...
async def copy_pinned_file_to_desk(chat_id, target_name):
    ...
    try:
        ...
        if msg:
            ...
            if copied.video: return copied.video.file_id
            elif copied.document: return copied.document.file_id
            elif copied.photo: return copied.photo.file_id
            return "none"
    except Exception as e:
        print(f"[copy_pinned_file_to_desk] Failed: {e}")
    return "none"

def server_list_nodes_verbose(account):
    dummy_id = "".join(random.choices(string.ascii_lowercase, k=5))
    try:
        out = run_server_command(account, ["kaggle", "kernels", "list", "--user", account["user"], "--csv"], dummy_id, timeout=45)
        if out.returncode != 0:
            return [], f"Node #{account['idx']} list fail."
        lines = out.stdout.strip().splitlines()
        refs = [line.split(",")[0].strip() for line in lines[1:] if line.split(",")]
        return refs, None
    except Exception as e:
        return [], f"Node #{account['idx']} exception: {e}"

def server_delete_node_verbose(account, ref):
    dummy_id = "".join(random.choices(string.ascii_lowercase, k=5))
    try:
        res = run_server_command(account, ["kaggle", "kernels", "delete", "-k", ref], dummy_id, timeout=45)
        return (True, None) if res.returncode == 0 else (False, f"Delete failed on {ref}")
    except Exception as e:
        return False, str(e)

def server_node_status(account, ref):
    dummy_id = "".join(random.choices(string.ascii_lowercase, k=5))
    try:
        out = run_server_command(account, ["kaggle", "kernels", "status", ref], dummy_id, timeout=30)
        return out.stdout.strip()
    except Exception as e:
        return str(e)

def kill_all_nodes_verbose():
    deleted, errors = [], []
    for acc in KAG_ACCOUNTS:
        refs, err = server_list_nodes_verbose(acc)
        if err:
            errors.append(err)
            continue
        for ref in refs:
            ok, derr = server_delete_node_verbose(acc, ref)
            if ok: deleted.append(ref)
            else: errors.append(derr)
    return deleted, errors

def server_push_node(account, slug, payload: dict, hw_mode: str, task_id):
    workdir = f"/tmp/{slug}"
    ...
    out = run_server_command(account, ["kaggle", "kernels", "push", "-p", workdir], task_id, timeout=90)
...
async def account_worker(account):
    ...
    while True:
        ...
        try:
            ...
            status_text = (
                f"🚀 **Task dispatched to Cloud Processing Node!**\n\n"
                f"⚡ **Acceleration:** `{hw_mode.upper()}` Engine\n"
                f"⚙️ *High-speed media environment initializing...*"
            )
            try:
            ...
            ok, log = await asyncio.to_thread(server_push_node, account, slug, task["payload"], hw_mode, task_id)
            if not ok:
                await report_error(task, f"Node initialization failed.\n{log[-400:]}")
                continue

            for _ in range(2160):
                if running_tasks.get(task_id, {}).get("cancel"):
                    await asyncio.to_thread(server_delete_node_verbose, account, f"{account_user}/{slug}")
                    break
                st = await asyncio.to_thread(server_node_status, account, f"{account_user}/{slug}")
...
async def report_error(task, text):
    try:
        ...
        try: await app.send_message(task["chat_id"], f"❌ **Error Occurred:**\n\n{text}")
        except Exception: pass
...
# --- COMMANDS ---
@app.on_message(filters.command(["start", "help", "stats", "addposition", "admark", "deletmark", "addfont", "removefont"]))
async def general_cmds(c, m: Message):
    ...
    if cmd == "help":
        help_text = (
            "🤖 **High-Speed Bot Usage Guide:**\n\n"
            "1️⃣ **Compress Video:** Video par reply karein `/1080s`, `/720s`, ya `/480s`\n"
            "2️⃣ **Hardsub Video:** Video par reply karein `/hsubs`\n"
            "3️⃣ **Set Watermark Position:** `/addposition left` ya `/addposition right`\n"
            "4️⃣ **Clean Setup State:** `/clean` type karein."
        )
        ...
        await m.reply(
            f"📊 **System Status & Diagnostics:**\n\n"
            f"🖥️ CPU: `{cpu}%`\n"
            f"💾 RAM: `{ram.percent}%`\n"
            f"👥 Active Nodes: `{len(KAG_ACCOUNTS)}`\n"
            f"🔥 Busy Runners: `{busy}`\n"
            f"⏳ Queued Tasks: `{task_queue.qsize()}`\n"
            f"⚡ Hardware Acceleration: `{current_hw_mode().upper()}`"
        )
        ...
        if not m.reply_to_message or not (m.reply_to_message.photo or m.reply_to_message.document):
            return await m.reply("❌ Valid file par reply karein.")
        ...
        await pinned.pin()
        await m.reply("✅ Registry saved successfully.")
...
RES_CMD_MAP = {"1080s": "1080p", "720s": "720p", "480s": "480p"}

@app.on_message(filters.command(["1080s", "720s", "480s"]))
async def compress_cmd(c, m: Message):
    ...
    if not media:
        return await m.reply("❌ Compression ke liye valid video par reply karein.")

    cmd = RES_CMD_MAP[m.command[0].lower()]
    orig_name = getattr(media, "file_name", "output.mp4")

    st = await m.reply("⏳ **Registering Task...** Securing media channel...")
...
@app.on_message(filters.command("hsubs"))
async def hsub_cmd(c, m: Message):
    if not await check_command_privacy(c, m): return
    media = m.reply_to_message.video or m.reply_to_message.document or m.reply_to_message.animation if m.reply_to_message else None
    if not media:
        return await m.reply("❌ Hardsub ke liye video/document par reply karein.")
    ...
    try:
        ...
        return await st_copy.edit(f"❌ Copy Error: `{e}`")

    await m.reply("📝 Subtitle file (.vtt/.srt/.ass) reply karein ya `S` type karke skip karein.")
...
async def prompt_watermark_or_execute(c, m, user_id, session):
    ...
    if wm_link != "none":
        ...
        await m.reply("🖼️ Watermark add karna hai? Type `A` for Add ya `S` for Skip.")
    else:
...
async def replies_controller(c, m: Message):
    ...
    if state == "WAIT_SUB":
        if m.document and m.document.file_name and m.document.file_name.lower().endswith(('.srt', '.ass', '.vtt', '.txt')):
            ...
            await m.reply("✏️ Video rename karni hai? Type `R` for Rename ya `S` for Same Name.")
        elif text == "S":
    ...
    if state == "WAIT_RENAME_CHOICE":
        if text == "R":
            ...
            await m.reply("✍️ Naya file name bhejo (without extension):")
        elif text == "S":
        ...
        if not m.text: return await m.reply("❌ Invalid name.")
        ...
        else: return await m.reply("❌ Invalid! Type `A` to add ya `S` to skip.")
        await execute_dispatch_hardsub(user_id, m)

async def execute_dispatch_hardsub(user_id, msg: Message):
    data = users_data.pop(user_id)
    st = await msg.reply("⏳ **Task Registered!** Queuing for processing node...")
...
async def kill_cmd(_, m: Message):
    ...
    if not KAG_ACCOUNTS: return await m.reply_text("❌ No cloud runners configured.")

    msg = await m.reply_text("🗑️ Aborting active runner tasks...")
    for tid in list(running_tasks.keys()):
        running_tasks[tid]["cancel"] = True

    deleted, errors = await asyncio.to_thread(kill_all_nodes_verbose)
    text = f"✅ `{len(deleted)}` Active node instance(s) purged."
...
async def clean_cmd(_, m: Message):
    ...
    await m.reply_text("🔄 **Session reset complete.**" if had else "🔄 **Already clean.**")
...
if __name__ == "__main__":
    ...
    async def run_bot_safe():
        ...
        try:
            ...
            print(f"[startup] Session string ready.")

            deleted, _ = await asyncio.to_thread(kill_all_nodes_verbose)
            print(f"[startup] {len(deleted)} active nodes cleaned.")

            for acc in KAG_ACCOUNTS:
                asyncio.create_task(account_worker(acc))
            print(f"[startup] {len(KAG_ACCOUNTS)} Node workers started.")

            await idle()
            await app.stop()
            
        except FloodWait as e:
            print(f"⚠️ Telegram FloodWait: {e.value}s")
            await asyncio.sleep(e.value + 5)
            await app.start()
            for acc in KAG_ACCOUNTS: asyncio.create_task(account_worker(acc))
            await idle()
        except Exception as e:
